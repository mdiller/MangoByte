...
class Audio(MangoCog):
    ...
    async def preprocess_tts_message(self, content, message):
        """Pre-processes a TTS message to handle links, GIFs, files, and custom emojis."""
        guildinfo = botdata.guildinfo(message.guild)
        author = message.author

        name = author.name
        if guildinfo.usenickname and author.nick:
            name = author.nick
        ...
        # If the content is solely a URL, handle GIF vs link cases
        url_only_match = re.match(r'^(https?://\S+)$', stripped)
        if url_only_match and not message.attachments:
            url = url_only_match.group(1)
            if url.lower().endswith('.gif'):
                return f"{name} sent a gif"
            else:
                domain = urlparse(url).netloc
                return f"{name} sent (a link to {domain})"

        # Replace custom emojis: <:name:id> or <a:name:id> -> "name emoji"
        content = re.sub(r'<a?:(\w+):\d+>', r'\1 emoji', content)

        # Replace any remaining links inline with "(a link to domain)"
        def replace_link(m):
            domain = urlparse(m.group(0)).netloc
            return f"(a link to {domain})"
        content = re.sub(URL_PATTERN, replace_link, content)

        # Handle file attachments
        if message.attachments:
            if content.strip() == "":
                content = f"{name} sent (a file)"
            else:
                content += " (a file)"
    ...
    async def on_message(self, message: disnake.Message):
        if message.guild and message.author.id != self.bot.user.id:
            if (message.content == "" or message.clean_content == "") and not message.attachments:
            ...
            if should_tts_message:
                ...
                try:
                    ...
                    tts_content = await self.preprocess_tts_message(message.clean_content, message)
                    if not tts_content or not tts_content.strip():
                        return
                    if guildinfo.simpletts:
                        await self.do_tts(tts_content, message.guild)
                    else:
                        await self.do_smarttts(tts_content, message.guild)
